document.addEventListener("DOMContentLoaded", function () {
  ...
  rocketsButton.addEventListener('click', function () {
      navigateTo('rockets');
  });
...
function navigateTo(section) {
  const sections = ["home", "launches", "rockets", "company"];
  sections.forEach(s => {
...
async function displayLaunches() {
  ...
  launchesSection.style.display = "block";
}
...
async function displayRockets() {
  ...
  rocketsData.forEach((rocket) => {
      ...
      rocketInfo.innerHTML = `
          ...
          <a href="#" onclick="displayRocketDetails('${rocket.id}')">Details</a>
      `;
